...
export const traversalScenario: Scenario = {
  ...
  tasks: [
    ...
    },
    {
      ...
      details:
        "Finora hai modificato l'URL. Ma un attaccante può cambiare qualunque dato che manda al server: il body di una richiesta API, un header, il nome di un file che carica. Se quel dato finisce dentro l'apertura di un file, il problema è lo stesso.\n\nQui lo scenario è questo:\n\n1. L'app salva i file di ogni utente in una sua cartella: /var/www/html/uploads/user-42.\n2. Tu mandi un JSON con il campo filename, ad esempio \"avatar.png\".\n3. Il server prende la tua cartella e ci attacca il filename che hai mandato, senza controllare dove finisce il percorso.\n\nQuindi ti basta scrivere ../ abbastanza volte per tornare alla radice, poi il percorso del file che vuoi leggere — in questo caso la chiave SSH privata di un altro utente.\n\nCome si evita: il client non deve mai mandare un nome di file. Nel database ogni file ha un numero (id): il client manda solo quel numero, e il server da solo calcola il percorso vero, che l'utente non vede né cambia.",
